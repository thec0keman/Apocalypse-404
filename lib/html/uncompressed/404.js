stars = (function(){
  ...
  Star_el.prototype.animate = function() {
    ...
    width = this.frame === 0 ? this.size * .8 : ( this.frame > 3 ) ? this.size : this.size * ( .7 + .2 * ( this.frame / 3 ) );

    canvas.beginPath();
    canvas.arc( this.x, this.y, width, 0, Math.PI*2, true);
    canvas.closePath();
  ...
  Explosion.prototype.animate = function() {
    ...
    canvas.beginPath();
    canvas.arc( this.x, this.y, 5 + (this.frame*2) , 0, Math.PI, true);
    canvas.closePath();
    canvas.fillStyle = 'rgba(255, 163, 0, ' + (35 - this.frame) / 35 + ')';
    canvas.fill();
    ...
    for( t=this.sparks.length-1; t>=0; t--) {
      ...
      // draw
      canvas.moveTo( spark.x1, spark.y1 );
      canvas.lineTo( spark.x2, spark.y2 );
    }
  ...
  Star.prototype.generate = function() {
    ...
    if ( this.id < NUMBER_ORBS * .4 )
      this.distance = gen_per(MIN_DISTANCE, 15);
    else if ( this.id < NUMBER_ORBS * .75 )
      this.distance = gen_per(10, MID_DISTANCE);
    else
      this.distance = gen_per(18, MAX_DISTANCE);
    ...
    this.explosion = null;
    this.trails.push( new Star_el(this.size, this.x, this.y) );
  }
  Star.prototype.animate = function() {
    ...
    // Animate
      ...
      if ( this.x > boundary_left && this.y < boundary_bottom) {
        trail = new Star_el(this.size, this.x, this.y);
        this.trails.push( trail );

        // Move
        this.speed.y += STAR_GRAVITY * this.distance;
        this.x -= this.speed.x;
        this.y += this.speed.y;
      }
      ...
      if ( this.distance >= DISTANCE_TO_FRONT && this.y > boundary_bottom && this.x > boundary_left && this.explosion !== "finished" ) {
        if ( this.explosion === null )
          this.explosion = new Explosion( this.x, this.y + 6 );
